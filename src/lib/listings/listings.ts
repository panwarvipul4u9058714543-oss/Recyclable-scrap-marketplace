...
export async function createListing(
  ...
  // Notify any collector whose ACTIVE route matches this new listing. Errors
  // here must not fail the create — the listing itself is the durable
  // artefact and notifications can be regenerated by re-calling the fan-out.
  try {
    await fanOutForNewListing(row.id);
  } catch {
    // Best-effort; a later fan-out (or a scheduled sweep) can retry.
  }
