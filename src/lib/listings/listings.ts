...
import { fanOutSavedSearchesForNewListing } from "@/lib/bulk/saved-searches";
...
export async function createListing(
  ...
  // Best-effort notification fan-outs on the write path. Failures must not
  // fail the create — the listing itself is the durable artefact and every
  // fan-out is idempotent, so a later call (or a scheduled sweep) can retry.
  try {
    await fanOutForNewListing(row.id);
  } catch {
    // ignored
  }
  try {
    await fanOutSavedSearchesForNewListing(row.id);
  } catch {
    // ignored
  }
  await recordEvent({
